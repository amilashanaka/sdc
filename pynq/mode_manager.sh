...
init_mode() {
    ...
    # Initialize database flags table if not exists
    mysql -u root -pdaq daq -e "CREATE TABLE IF NOT EXISTS flags (id INT NOT NULL AUTO_INCREMENT, run_mode INT DEFAULT 0, PRIMARY KEY (id) USING BTREE);" 2>/dev/null || true
    
    # Insert default row if not exists
    mysql -u root -pdaq daq -e "INSERT IGNORE INTO flags (run_mode) VALUES (0);" 2>/dev/null || true
}
...
set_mode() {
    ...
    # Update database flags table (run_mode: 0=RUN, 1=DEBUG)
    local db_flag=0
    if [ "$mode" = "DEBUG" ]; then
        db_flag=1
    fi
    
    # Update the flags table - set run_mode to the flag value
    mysql -u root -pdaq daq -e "UPDATE flags SET run_mode=$db_flag WHERE id=1; INSERT INTO flags (run_mode) VALUES($db_flag) ON DUPLICATE KEY UPDATE run_mode=$db_flag;" 2>/dev/null || true
